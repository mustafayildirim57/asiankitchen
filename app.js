...
const categoryItems = menu.reduce( 
  (categoryItem, item) =>  {
    if (!categoryItem.includes(item.category)){
...
const catagoryList =() => { 
  const catagoryButtons = categoryItems.map(category => {
    return `<button class="btn btn-outline-dark btn-item" data-id="${category}">${category}</button>`
  })
  .join("")

  buttonDom.innerHTML = catagoryButtons
  const filtersbtnDom = document.querySelectorAll(".btn-item")  
  
  //filtre
  filtersbtnDom.forEach((btn) => {
